...
import com.example.bismillah.databinding.ActivityHomeworkBinding
        ...
        if (result.data != null) {
            when (result.resultCode) {
                AddHomeworkActivity.RESULT_ADD -> {
                    val homework = result.data?.getParcelableExtra<Homework>(AddHomeworkActivity.EXTRA_HOMEWORK)
                    if (homework != null) {
                ...
                AddHomeworkActivity.RESULT_UPDATE -> {
                    val homework = result.data?.getParcelableExtra<Homework>(AddHomeworkActivity.EXTRA_HOMEWORK)
                    val position = result.data?.getIntExtra(AddHomeworkActivity.EXTRA_POSITION, -1) ?: -1
